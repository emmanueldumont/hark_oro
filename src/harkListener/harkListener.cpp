...
void process_Sources(t_sourceValues * srvVal, int size)
{
  ...
  if(size > 0)
  {
    ...
    {
      bool next = false; // Boolean which indicate if we can skip next parts or keep going in treatment
      //-------------------------------
...
void clearSource(int id)
{
  ...
  ss.str("");
  ss << "BigBrother#"<< enumCmd <<"#s"<< id <<"#?x#?y";
...
void updateSourceData(int id, char data, int value)
{
  ...
  ss << "BigBrother#"<<enumCmd<<"#s"<< id <<"#"<< data <<"#?y";
  std_msgs::String msg;
  msg.data = ss.str();
  oroChatterSender(msg);
  ...
  ss << "BigBrother#" << enumCmd <<"#s"<< id << "#"<< data <<"#"<< value;
...
void updateSourceAngle(int id, int angle)
{
  ...
  ss.str("");
  enumCmd = (char)CMD_CLEAR;
  ss << "BigBrother#"<<enumCmd<<"#s"<< id <<"#angle#?y";
  std_msgs::String msg;
  msg.data = ss.str();
  oroChatterSender(msg);
  ...
  ss.str("");
  enumCmd = (char)CMD_ADD_PROP;
  ss << "BigBrother#" << enumCmd <<"#s"<< id << "#angle" <<"#"<< angle;
...
void updateTimestamp(int id)
{
  ...
  ss.str("");
  enumCmd = (char)CMD_CLEAR;
  ss << "BigBrother#"<<enumCmd<<"#s"<< id <<"#timestamp#?y";
  std_msgs::String msg;
  msg.data = ss.str();
  oroChatterSender(msg);
  ...
  ss.str("");
  enumCmd = (char)CMD_ADD_PROP;
  ss << "BigBrother#" << enumCmd <<"#s"<< id << "#timestamp" <<"#"<< timeinfo->tm_hour <<"."<< timeinfo->tm_min;
  ...
  for(int cpt=0; cpt < 7; cpt++)
  {
    ...
    switch(cpt)
    {
      ...
      case 0: ss.str("");
              ...
              ss << "BigBrother#"<<enumCmd<<"#s"<< soundSource->id <<"#Sound";
      break;
      
      // 1- Add that we can hear it
      case 1:  ss.str("");
	            enumCmd = (char)CMD_ADD_PROP;
				      ss << "BigBrother#" << enumCmd <<"#"<< gName << "#canHear" <<"#s"<< soundSource->id;
      break;
      ...
      case 2:  // Gettime
              ...
              ss.str("");
              enumCmd = (char)CMD_ADD_PROP;
			        ss << "BigBrother#" << enumCmd <<"#s"<< soundSource->id << "#timestamp" <<"#"<< timeinfo->tm_hour <<"."<< timeinfo->tm_min;
      break;
      
      // 3- Add the sound's X-position
      case 3: ss.str("");
              ss << "BigBrother#"<<enumCmd<<"#s"<< soundSource->id <<"#x#"<< soundSource->x;
      break;
      
      // 3- Add the sound's Y-position
      case 4: ss.str("");
              ss << "BigBrother#"<<enumCmd<<"#s"<< soundSource->id <<"#y#"<< soundSource->y;
      break;
      
      // 3- Add the sound's Z-position
      case 5: ss.str("");
              ss << "BigBrother#"<<enumCmd<<"#s"<< soundSource->id <<"#z#"<< soundSource->z;
      break;
      
      // 3- Add the sound's Angle-position
      case 6: ss.str("");
              ss << "BigBrother#"<<enumCmd<<"#s"<< soundSource->id <<"#angle#"<< soundSource->angle;
      break;
